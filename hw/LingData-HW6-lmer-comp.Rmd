...
```{r 1.1, echo = FALSE}
upsid %>%
  ggplot(aes(x = consonants, y = vowels)) +
  geom_point() +
  theme_bw()
```
...
```{r 1.2, echo = FALSE}
fit1 <- lm(data = upsid, formula = vowels ~ consonants)
summary(fit1)
```
...
```{r 2.1, echo = FALSE}
upsid %>%
  ggplot(aes(x = consonants, y = vowels, color = area)) +
  geom_point() +
  theme_bw() +
  stat_ellipse()
```
...
summary(fit2)
```
The _consonants_ variable is still significant, but its p-value has increased after adding the _area_ random effect. We can see that there are some parallel lines on the graph because of them having the same coefficients (slope) and a different intercept (after adding the random effect).
...
summary(fit3)
```
The _constants_ variable is no longer significant, because its p-value now equals 0.48 >> 0.05. The lines on the graph are similar to the lines on the previous graph, but now they do not necessarily have the same slope, and some of them have different lengths as well.
...
upsid %>%
  ...
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit4), aes(x = consonants, y = .fitted), color = "blue")
```
...
summary(fit4)
```
The _constants_ variable is still insignificant, although its p-value has decreased a little. In this last model, each group of languages by area has its own slope and its own intercept.